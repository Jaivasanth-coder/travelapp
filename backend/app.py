...
# ─────────────────────────────────────────
# ENVIRONMENT LOADING
# Local  : reads env.properties next to app.py or in project root
# Render : env vars are injected directly — no file needed
# ─────────────────────────────────────────
_here = Path(__file__).resolve().parent
_env_file = None
for _candidate in [
    _here / ".env",
    _here / "env.properties",
    _here.parent / ".env",
    _here.parent / "env.properties",
]:
...
print(f"  ENV FILE : {_env_file or 'None (using injected env vars)'}")
print(f"  DATABASE : {'Supabase (cloud)' if 'supabase' in _db_url else 'localhost' if 'localhost' in _db_url else _db_url[:40] + '...' if _db_url else 'NOT SET ❌'}")
print("=" * 50)
...
def get_db():
    """
    Connect using DATABASE_URL directly as a DSN — psycopg2 handles
    percent-encoded characters (%40 etc.) natively so no manual parsing needed.
...
def seed_destinations(cur, conn):
    ...
    if cur.fetchone()[0] == 0:
        ...
        cur.executemany(
            "INSERT INTO destinations (name, country, description, image_url, category) VALUES (%s,%s,%s,%s,%s)",
            destinations
        )
    ...
    if cur.fetchone()[0] == 0:
        ...
        cur.executemany(
            "INSERT INTO packages (title,destination_id,description,image_url,price,duration_days,max_persons,includes,excludes,itinerary,category,is_featured,rating) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
            packages
        )
...
def register():
    ...
    cur.execute(
        "INSERT INTO users (name, email, password) VALUES (%s,%s,%s) RETURNING id, name, email, is_admin",
        (name, email, hashed)
    )
    user = cur.fetchone()
...
def create_package():
    ...
    cur  = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cur.execute(
        "INSERT INTO packages (title,destination_id,description,image_url,price,duration_days,max_persons,includes,excludes,itinerary,category,is_featured) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING *",
...
def update_package(pkg_id):
    ...
    cur  = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cur.execute(
        "UPDATE packages SET title=%s,description=%s,image_url=%s,price=%s,duration_days=%s,max_persons=%s,includes=%s,excludes=%s,itinerary=%s,category=%s,is_featured=%s WHERE id=%s RETURNING *",
...
def create_booking():
    ...
    cur.execute(
        "INSERT INTO bookings (user_id,package_id,travel_date,num_persons,total_price,special_requests) VALUES (%s,%s,%s,%s,%s,%s) RETURNING *",
...
def get_my_bookings():
    ...
    cur  = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cur.execute(
        "SELECT b.*, p.title as package_title, p.image_url, p.duration_days FROM bookings b JOIN packages p ON b.package_id = p.id WHERE b.user_id = %s ORDER BY b.created_at DESC",
        (request.user_id,)
    )
    result = cur.fetchall()
    cur.close(); conn.close()
...
def update_booking_status(booking_id):
    ...
    cur  = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cur.execute(
        "UPDATE bookings SET status=%s, payment_status=%s WHERE id=%s RETURNING *",
...
def create_enquiry():
    ...
    cur  = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cur.execute(
        "INSERT INTO enquiries (name,email,phone,message) VALUES (%s,%s,%s,%s) RETURNING *",
...
@app.route("/api/enquiries/<int:enq_id>/status", methods=["PUT"])
@admin_required
def update_enquiry_status(enq_id):
    data = request.json
    conn = get_db()
    cur  = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cur.execute("UPDATE enquiries SET status=%s WHERE id=%s RETURNING *", (data.get("status"), enq_id))
    enq = cur.fetchone()
    conn.commit(); cur.close(); conn.close()
    return jsonify(dict(enq))
...
def ping():
    """
    Lightweight public endpoint for external uptime monitors (UptimeRobot etc.)
    No DB call — just confirms the process is alive.
    """
    return "pong", 200
